...
void setup_scene(){
  ...
  /****
	   ...
	   Thus, in order to correctly render the shapes, the y-coordinates are multiplied by -1.
   ****/
  
  glBegin(GL_TRIANGLES);
  for(int i = 0; i<spaceship.size(); i++){
	glNormal3f(-1*spaceship[i].normal[0], spaceship[i].normal[1], spaceship[i].normal[2]);
	if((spaceship[i].normal[2] == -1) && (spaceship[i].vertices[0][2] < -1.5)) glColor3f(.4, .6, 1);
	for(int j = 2; j>=0; j--){
	  glVertex3f(-1*spaceship[i].vertices[j][0], spaceship[i].vertices[j][1], spaceship[i].vertices[j][2]);
	}
	glColor3f(0.4, 0.4, 0.4);
  }
  glEnd();
...
void display(){
  ...
  // Set up camera position and movement
  gluLookAt(0.0, 0.0, -5.0,
			xpos, ypos, 0,
			0.0, 0.0, 1.0);
  //setup_scene();
